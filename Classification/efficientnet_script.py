...
def create_model(num_classes):
    """Creates and returns EfficientNetV2 model."""
    base_model = EfficientNetV2B0(include_top=False, input_shape=(256, 256, 3), weights='imagenet')
...
def main():
    parser = argparse.ArgumentParser(description="Train or evaluate EfficientNetV2 on a dataset.")
    parser.add_argument('--mode', type=str, required=True, choices=['train', 'evaluate'], help='Mode to run the script in.')
    parser.add_argument('--data_dir', type=str, required=True, help='Directory containing training/validation data.')
    ...
    if args.mode == 'train':
        ...
        evaluate_model('efficientnetv2_model.h5', args.data_dir + '/test', args.batch_size)
